...
<h4 style="background-color: rgb(78, 78, 255); margin:0px; color:white; padding:10px; border-radius:0px 0px 20px 20px;">Order dulu</h4>
@if(session('success'))
...
<div class="container-daftar-order">
        ...
        <div class="card" style="text-align: star; padding:10px; margin:10px 0px; border:0; border-radius:20px;">
            ...
            {{-- <div class="search-menu-order"> --}}
             ...
             <div class="pilih-menu">
                <div class="card Minuman-btn" style="box-shadow: 0px 2px 5px gray; cursor:pointer;" onclick="showMenu('Minuman')">Minuman</div>
                <div class="card Makanan-btn" style="box-shadow: 0px 2px 5px gray; cursor:pointer;"  onclick="showMenu('Makanan')">Makanan</div>
          ...
          <div class="col menu-item {{ $item_menu->kategori }}" style="margin: 10px">
              <div class="col">
                  <div class="card" style="width: 9rem;">
                      <img src="assets/coffee.jpeg" class="card-img-top" alt="...">
                      <div class="card-body">
                          <b>{{ $item_menu->nama }}</b>
                          <p class="card-text">{{ $item_menu->harga }}</p>
                          <button class="addToCartBtn" data-id="{{ $item_menu->id }}" data-name="{{ $item_menu->nama }}" data-price="{{ $item_menu->harga }}" style="background-color: rgb(99, 99, 199); border:0; border-radius:5px; color:white;">Pilih</button>
                      </div>
                  </div>
              </div>
          </div>
    ...
    <div class="cart">
        ...
        <div class="d-grid gap-2">
          <button id="orderButton" class="btn" type="submit" style="background-color: rgb(74, 155, 74); color:white;">Tambah Orderan</button>
      </div>
...
searchInput.addEventListener('input', function () {
  ...
  cards.forEach(function (card) {
    const cardName = card.querySelector('b').textContent.toLowerCase(); // Ambil nama menu dari setiap kartu dan ubah ke huruf kecil
...
// ---------------------------------------------
 ...
 // Bagian dari kode JavaScript sebelumnya
...
function renderCart() {
    ...
    cart.forEach(item => {
        ...
        menuInputValue += item.name + ', '; // Menggunakan pemisah tertentu jika diperlukan
